...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/spin_glass}
	\caption{spin glass}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/ferro1}
	\caption{ferro1}
	\label{fig:ferro1}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/ferro2}
	\caption{ferro2}
	\label{fig:ferro2}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/ferro3}
	\caption{ferro3}
	\label{fig:ferro3}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/antiferro1}
	\caption{antiferro1}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{figs/antiferro2}
	\caption{antiferro2}
...
\begin{figure}[H]
	...
	\end{minipage}
	\caption{Магнитный структурный фактор $I(\vec{q})$ для ферромагнитного (слева) и антиферромагнитного (справа) упорядочение в решетке из 64 спинов.}
	\label{fig:msf_ferro_anti}
\end{figure}

На рисунке \ref{fig:msf_ferro_anti} изображен  магнитный структурный фактор $I(\vec{q})$ в обратном пространстве $(q_x, q_y)$ для ферромагнетика (слева) и антиферромагнетика (справа). Белые пунктирные линии - граница зоны Бриллюэна ($\pm\pi$). Положения пиков $I(\vec{q})$ абсолютно подтверждают тип магнитного порядка в системах. Для ферромагнетика пик наблдается в центре $(0,0)$, для антиферромагнетика -- на границах зоны Бриллюэна в точках $(\pm\pi,\pm\pi)$ \ref{luscombe1996nonequilibrium}. 
...
\begin{figure}[H]
	...
	\end{minipage}
	\caption{Магнитный структурный фактор $I(\vec{q})$ для конфигураций вырожденного основного состояния спинового стекла Эдвардса-Андерсона ($P_+=0.5$) при разном спиновом избытке ($M_{gs}=-2,-8,-14$) слева и соответствующая конфигурация спиновых переменных справа.}
	\label{fig:msf}
\end{figure}

На рисунке \ref{fig:msf} слева изображен магнитный структурный фактор в обратном пространстве $(q_x, q_y)$ для нескольких конфигураций вырожденного основного состояния спинового стекла Эдвардса-Андерсона ($M_{gs}=-2,-8,-14$). Справа показана соответсвующая $I(\vec{q})$ конфигурация спиновых переменных. Белые и черные круги - $S_i=\pm1$, прямые и зубчатые линии - ферромагнитные и антиферромагнитые связи, обведенные пары узлов - фрустрированные пары.

Так как спиновое стекло характеризуется разупорядченным положением спинов за счет конкурирующих взаимодействий, то пики $I(\vec{q})$ в обратном пространстве размыты, расположены хаотично и отличаются для разных конфигураций спиновых переменных соответствующих одному и тому же основному состоянию. Тем не менее, на рисунке \ref{fig:msf} снизу для $I(\vec{q})$ при $|M_{gs}| = 14$ мы можем видеть, что помимо большого количества хаотично расположенных пятен, также наблюдается пятна в точках $(\pm\pi,\pm\pi)$ и продолговатая область с высоким значением $I(\vec{q})$ и центром в точке $(0,0)$. То есть в каком-то виде происходит ферро- и антиферромагнитное упорядочение одновременно. Это является следствием того, что в конфигурации основного состояния спинового стекла образуются ферромагнитные и антиферромагнитные кластеры.
